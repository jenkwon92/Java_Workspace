...
AWT 에서 지언하는 배치는 다음과 같이 5가지 유형이 있다
1) FlowLayout: 행방향으로 늘어서는 배치방식, 만일 공간이 부족하면 다음 행으로 내려감
						컨테이너 크기에 따라 흐르듯 배치가 되므로 flow라는 뜻이 부여됨
2) BorderLayout: 북,동, 서, 남, 중앙 이라는 방위를 갖는 배치방법
3) GridLayout: 격자 즉, 바둑판 모양의 배치방법
4) CardLayout:  카드처험 하나의 카드가 보일때는 다른 카드가 안보이는 방식의 배치
5)GridBagLayout: GridLayout 을 보다 상세하게 제어할 수 있는 배치방식
...
class BorderTest{
	public static void main(String [] args){
		...
		frame.add(bt_west, BorderLayout.WEST);  //서족
		frame.add(bt_east, BorderLayout.EAST); //동쪽
		//센터는 명시하지않아도 디폴트이기때문에 생략해도 적용된다
		frame.add(bt_center, BorderLayout.CENTER); //가운데
