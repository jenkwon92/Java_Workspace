...
public class Member extends JPanel {
   ...
   boolean overlap = false;// 락커 중복확인해줄 변수
...
//         memberList.column = columnNames; // 컬럼 정보 대입
   ...
   // 락커가 중복되어도 id가 같다면 중복 허용
   public void updateMember() {
         PreparedStatement pstmt =null;
         ResultSet rs =null;
         
         String sql="select m_id from gym_member where m_locker="+ t_locker.getText();
         
         try {
            pstmt=mainPage.getCon().prepareStatement(sql);
            rs=pstmt.executeQuery();
            while(rs.next()) {            
               if(rs.getInt("m_id")==Integer.parseInt(value)) {
                  overlap=true;
               }else {
                  JOptionPane.showMessageDialog(this, "사용중인 락커키입니다.");
                  overlap=false;
               }
            }
         } catch (SQLException e) {
