...
public class MyCanvas extends Canvas{
	...
	Image img; //얘도 추상임 근데 툴짓을 통해 인스턴스를 얻을 수 있음
	public MyCanvas() {
		kit = Toolkit.getDefaultToolkit(); //자체 클래스 메서드(static)를 통해 인스턴스 얻는 방식
		img = kit.getImage("D:/workspace/java_workspace/project/res/travel2/aa.jpg"); //윈도우 os
	}
	...
	public void paint(Graphics g) {
		...
		//단지 색상만 관여하는것이아니라
		//도형, 텍스트, 이미지, 선, 점 등 다양한 그래픽 처리를 이한 기능을 지원하는 실질적 그래픽 처리 담당자이다.
		...
		// 		일반, 추상, 인터페이스
		//조사해보니 이미지는 추상클래스이다
		//실망x:추상클래스인경우, 개발자가 자식클래스오 구현 후 , 자식을 new해서 사용해야함이 원칙이지만
		//		sun사가 제공해주는 대부분의 추사을래스는 사실, 이미 인스턴스를 생성해놓고 간접적으로 쉽게 
		//		얻는 메서드를 제공해주거나 인스턴스를 얻을 수 있는 자체 메서드를 지원해주는 경우가 더 많다
		//시스템에 하드디스크 경로를 이용하여 이미지를 가져올 경우엔, 플랫폼을 통해 이미지 객체를 가져올 수 있는
		//Toolkit객체를 이용해야하므로, 이미지를 얻기 전에 우전 툴킷 객체의 인스턴스부터 얻자
		g.drawImage(img, 20, 20, this); //이미지 관찰자는 캔버스로하자
	}
}
