...
public class ButtonCollection extends JFrame implements ActionListener{
	...
	//배열은 크기를 정해야하기때문에 크기를 알수 없는 프로그램에선 사용에 제한이 있다.
	//JButton[] btn = new JButton[50]; //배열의 가장 큰 특징, 배열은 생성시 반드시 크기를 명시해야한다
	ArrayList<JButton> btn = new ArrayList<JButton>(); //크기가 0이지만 고무줄처럼 늘어남
	...
	public void create() {
		...
		btn.add(bt);//리스크에 추가하기! js의 push() 메서드와 같다
		System.out.println("현재까지 누적된 리스트(버튼)의 수는"+btn.size());
	...
	public void setBg() {
		...
		//ArrayList는 순서있는 집합이므로 , for문을 사용할 수있다.
