...
public class MessageThread extends Thread{
	...
	public void send(String msg) {
		try {
			//벡터에 들어있는 메시지 쓰레드 만큼 반복문 수행하면서, write, flush 수행하면됨 (멀티 케스팅)
			for(int i = 0; i<multiServer.clientList.size(); i++ ) {
				MessageThread messageThread= multiServer.clientList.get(i); //벡터에 들어있는 메세지쓰레드를 하나씩 꺼낸다
