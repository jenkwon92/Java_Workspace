...
public class EchoServer extends JFrame {
	...
	Thread thread; //메인쓰레드 대신, 접속자를 감지하게될 쓰레드(accept() 메서드때문에)
	...
	public void startServer() {
		try {
			...
			// 하지만 메인 쓰레드는 개발자가 생성하는 일반쓰레드와는 하는 역할에 차이가있다.
			// 메인쓰레드는 프로그램을 운영해주는 역할 특히, 그래픽 처리, 이벤트 처리까지 담당하므로, 절대로 아래의 업무금지
			// 1) 무한루프에 빠뜨리지 말것
			// 2) 대기상태에 빠지게 하지 말것! (accept(), 스트림의 read() 등등..)
			// 참고로 안드로이드에서는 메인쓰레들의 1,2 번 시도자체를 에러로 본다
