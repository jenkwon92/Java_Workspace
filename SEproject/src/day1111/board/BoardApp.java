...
public class BoardApp extends JFrame {
	...
	// 모든 페이지들이 사용할 접속정보 객체 선언 . 접속에 필요한 정보들
	...
	//회원정보를 보관할 자료형?
	//이 병수에 데이터가 채워지는 시점은? 로그인 성공할때 인스턴스 생성하자
	...
	public BoardApp() {
		...
		//디폴트로 보여질 페이지와 안보이게될 페이지에 대한 처리
		...
		// 참고> javascript에서는 람다를 클로저라한다
		// 이벤트 구현시 , 재정의할 메서드가 달랑 1개인 경우 굳이, 내부 익명클래스라는 표기를
	...
	public void loginCheck(int page) {
		if(hasSession == false) { //로그인 하지않은 상태임
	...
	public void getConnection() {
		try {
			...
			con = DriverManager.getConnection(url, user, pass); // 접속 시도후, 객체반환
			if (con == null) {
				JOptionPane.showMessageDialog(this, "데이터베이스에 접속하지 못했습니다");
			} else {// 접속성공의 경우 윈도위 제목창에 현재 접속자 출력
