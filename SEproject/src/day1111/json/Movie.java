...
public class Movie extends JPanel implements Runnable {
	...
	public Movie(JsonGallery jsonGallery, int width, int height, String url, String title, String phase,
		...
		thread = new Thread(this); // Runnable을 구현할 객체를 인수로 널어준다
	...
	public void run() {
		...
		// 2. 이미지가 클래스 패스상 즉, 패키지에 있을 경우 ClassLoader()로 이용
		// 3. BufferedImage> ImageIO
		try {
			URL path = new URL(url);
			buffImg = ImageIO.read(path);
			big = buffImg.getScaledInstance(400, 600, Image.SCALE_SMOOTH); //큰 이미지
