...
public class RegistForm extends JPanel {
	...
	public RegistForm(BoardApp boardApp) {
		...
		//System.out.println("나 태어날때 con :"+con);
	...
	public void regist() {
		...
		try {
			...
			//DML의 경우, 이 쿼리 수행에 의해 영향을 받은 레코드 수가 반환되므로, 만일 0이 반환된다면 실패로판단
			int result = pstmt.executeUpdate(); //DML의 경우에는 executeUpdate(), select는 executeQuery()
			if(result == 0) {
				JOptionPane.showMessageDialog(this, "가입에 실패하였습니다. \n관리자에 문의주세요");
			}else {
				JOptionPane.showMessageDialog(this, "가입에 성공하였습니다. 축하드려요");					
			}
