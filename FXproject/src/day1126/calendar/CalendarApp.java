...
public class CalendarApp extends Application {
	...
	Button bt;
	TilePane tilePane;
	int StartDayOfWeek; //해당 월의 시작요일(1부터 시작함)
	...
	public void start(Stage stage) throws Exception {
		...
		//루트하위에 등록된 객체 찾아나서기 
		//js의 document.getElementById("ch_yy");의 원리와 같다
	...
	public void initDate() {
		...
		//ch_yy.getSelectionModel().select(0);//n번째요소가 선택되어있게
		ch_yy.getSelectionModel().selectFirst(); //무조건 처음요소 선택되어있게
	...
	public void getLastDate() {
		...
		//주의!! 해당월을 0일로 세팅하면(즉, 날짜만 조작해도), 월이 자동으로 변환된다.
		cal.set(yy, mm, 0);
	...
	public void printData() {
		...
		//주의! 출력하기전에 유저가 선택한 년, 월을 이용하여 startDayOfweek, lastDate 를 다시 구해야함
		//따라서 아래의 메서드를 다시 호출하면됨
		getStartDayOfWeek();
		getLastDate();
		
		//각 월에 알맞는 데이터 출력
		//반복문에서 i의 역할은 날짜 출력이아니라. 단지 반복문의 횟수를 결정하기위함
